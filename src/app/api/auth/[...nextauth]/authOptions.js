...
export const authOptions = {
  ...
  callbacks: {
    /**
     * @param {object} param0
     * @param {{user: FrontUserWithToken}} param0.token
      ...
      if (user) {
        ...
        const userFromDb =
          role === 'student'
            ...
            : await getUserById(token.user.id);
        if (userFromDb) {
          const newRole = dbUserRoleToFrontUserRole(userFromDb.profile.role);
          token.user = adaptUserFromDb(userFromDb, newRole);
        }
      }
      return token;
    },
